...
export function PassivoSummaryHero({
  ...
  const rendPct =
    summary.valorBrutoTotal > 0 ? (summary.rendimentoTotal / summary.valorBrutoTotal) * 100 : 0;
  ...
  const stats = [
    ...
    { label: "Tributos totais", value: formatBRL(summary.irrfTotal + summary.iofTotal), tone: "text-ink" },
    { label: "Cotistas", value: formatNumber(summary.quantidadeCotistas), tone: "text-ink" },
  ];
    ...
    <section className="card hero-grad fade-up shrink-0 overflow-hidden rounded-[var(--radius)]">
      <div className="flex flex-wrap items-end justify-between gap-4 p-5 sm:p-6">
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm">
          <span className={cn("num inline-flex items-center gap-1 rounded-md px-2 py-1 font-bold", upRendimento ? "chip-gain" : "chip-loss")}>
            {upRendimento ? <ArrowUpRight className="h-3.5 w-3.5" /> : <ArrowDownRight className="h-3.5 w-3.5" />}
            {formatBRL(summary.rendimentoTotal)}
            <span className="font-semibold opacity-80">({formatPct(Math.abs(rendPct))})</span>
          </span>
