...
import {
  ArrowUp, BarChart3, CalendarRange, Layers, MessageSquare, RefreshCw, TriangleAlert,
} from "lucide-react";
...
const SUGGESTIONS = [
  ...
  { icon: BarChart3, label: "Concentração", text: "Qual a maior concentração por classe?" },
    ...
    <div className="card flex h-full min-h-0 flex-col overflow-hidden rounded-[var(--radius)]">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3.5">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[1rem] font-bold leading-none tracking-tight text-ink">Assistente Zarya</p>
        </div>
        {messages.length > 0 && (
      ...
      {/* Mensagens */}
      <div ref={scrollRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-[hsl(225_36%_98.5%)] px-4 py-4">
        {messages.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center gap-3 px-2 text-center">
            ...
            <div>
              <p className="text-[0.95rem] font-bold">Converse com seus dados</p>
              <p className="mx-auto mt-1 max-w-[16rem] text-sm leading-relaxed text-muted-foreground">
                Pergunte sobre a carteira. O copiloto troca data, carteira e atualiza os gráficos.
              </p>
            </div>
          </div>
        )}
      ...
      {/* Input */}
      <form onSubmit={(e) => { e.preventDefault(); submit(input); }} className="shrink-0 border-t border-border bg-panel p-3">
